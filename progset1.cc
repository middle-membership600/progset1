...
vector<vector<Node > > adjacency_list(vector<vector<float> > g){
  ...
  for (int i = 0; i < g.size(); i++) {
    ...
    for (int j = 0; j < g.size(); j++) {
      ...
      //float e = ((float)(log2(g.size())-7)/100);
     // float i = (((float) pow(2,g[i].size()-3))/(pow(1.13+e,log2(g.size()))));
     float w;

     if(g[i].size() == 2) {
     w = ( float)1*pow((0.60),log2(g.size())-7);
     }
     if(g[i].size() == 3) {
     w = ( float)1*pow((0.62248746188),log2(g.size())-7);
     }
     if(g[i].size() == 4) {
     w = ( float)1 *pow((0.62171433918),log2(g.size())-7);
...
int main(int argc, char *argv[]) {
    ...
    assert(result.isMST);
     ...
     time_t end = time(0);
     cout << "number of points: " << numpoints << " " <<"dimensions: " << dimension <<"\naverage weight: " << average << "\naverage deviation" << devi << "\n";
